...
const CashierUpdate = ({ cashierId, onUpdateSuccess, onCancel }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cashierResponse = await axios.get(`${API_BASE_URL}/cashiers/${cashierId}`, {
          headers: {
        ...
        const cashier = cashierResponse.data.data;
        setFormData({
          name: cashier.name || '',
          email: cashier.email || '',
          phone: cashier.phone || '',
          ...
          shift_start: cashier.shift_start ? cashier.shift_start.substring(11, 16) : '', // Extract HH:MM from datetime
          shift_end: cashier.shift_end ? cashier.shift_end.substring(11, 16) : ''
        });

        // Fetch available queues
        const queuesResponse = await axios.get(`${API_BASE_URL}/queues`, {
          headers: {
            'Authorization': `Bearer ${getAuthToken()}`,
            'Accept': 'application/json'
          }
        ...
        setMessage('Failed to load cashier data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${API_BASE_URL}/cashiers/${cashierId}`, formData, {
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
      ...
      if (error.response?.status === 422) {
        ...
        setMessage('Cashier not found.');
      } else {
        setMessage('An error occurred while updating the cashier.');
      }
    } finally {
      setLoading(false);
    }
